...
public class main{
    public static void Main(string[] args){
        ...
        List<double> x_raw_list = new List<double>{};//(not required for timing)
        ...
        for(string line=instream.ReadLine(); line!=null && linenr<maxN; line=instream.ReadLine()){  
            ...
            x_raw_list.Add(double.Parse(numbers[0]));//(not required for timing)
        ...
        if (signalCount > N) {
            x_raw_list.RemoveRange(N, signalCount - N);//(not required for timing)
            y_raw_list.RemoveRange(N, signalCount - N);
        }
        double[] x_raw = x_raw_list.ToArray();//(not required for timing)
